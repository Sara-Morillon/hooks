...
describe('useQuery', () => {
  it('should not be loading by default', () => {
    const queryFn = vi.fn<() => string>().mockResolvedValue('result')
  ...
  it('should execute query with params', async () => {
    const queryFn = vi.fn<(a: string) => string>().mockResolvedValue('result')
    const { result } = renderHook(() => useQuery(queryFn))
  ...
  it('should return query error', async () => {
    const queryFn = vi.fn<() => string>().mockRejectedValue('error')
    const { result } = renderHook(() => useQuery(queryFn))
    await act(() => result.current.execute())
